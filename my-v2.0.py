...
def show_message(params):
    global x,y,message_windows  # 使用全局字典
    ...
    if message not in message_windows or not message_windows[message].winfo_exists():
        ...
        message_windows[message].geometry(f"+{x}+{y}")
...
class WindowManager:
    def __init__(self, root):
        global x,y
        # 窗口标题
        self.TITLE = f"{TITLE} v{current_version}"
        # 定义窗口标题列表
    ...
    def open_settings_window(self):
        ...
        if not hasattr(self, 'settings_window') or not self.settings_window.winfo_exists():
            ...
            action_btn = tk.Frame(self.settings_window)
            action_btn.pack(padx=10,pady=10)
            self.check_update = tk.Button(action_btn, text="检查更新", command=check_and_update, cursor=f"@{btn_path}")
            self.check_update.pack(side=tk.LEFT,padx=10)
            # QQ 群按钮
            self.qq_group_button = tk.Button(action_btn, text="官方QQ群",command=lambda:webbrowser.open("https://qm.qq.com/q/JdiwPsYpCG"), cursor=f"@{btn_path}")
            self.qq_group_button.pack(side=tk.LEFT,padx=10)
    ...
    def show_code(self):
        ...
        if not hasattr(self, 'code_window') or not self.code_window.winfo_exists():
            ...
            self.code_window.geometry(f"+{x}+{y}")
            # 下载图片到本地
